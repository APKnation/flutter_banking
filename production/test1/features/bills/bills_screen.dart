...
class _BillsScreenState extends State<BillsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final billers = await _repo.getBillers();
      setState(() { _billers = billers; _loading = false; });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    final saved = _billers.where((b) => b.isSaved).toList();
                ...
                children: [
                  ...
                  if (saved.isEmpty)
                    const Padding(
                      ...
                      child: Text('No saved billers yet.', style: TextStyle(color: AppColors.textMuted)),
                    )
                  else
                    SizedBox(
                      height: 140,
                      child: ListView.builder(
                        ...
                        itemBuilder: (_, i) => _SavedBillerCard(biller: saved[i]),
                      ),
                    ),
                  const SizedBox(height: 32),
